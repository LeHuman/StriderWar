...
struct Dummy : DOS::Input::Interface {
    ...
    void update() {
        ...
        const Fixed &x = player->ship.x;
        const Fixed &y = player->ship.y;
        const Fixed &vx = player->ship.vx;
        const Fixed &vy = player->ship.vy;
        ...
        Fixed tx = target->ship.x;
        Fixed ty = target->ship.y;
